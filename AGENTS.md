...
8. **Cancellation is v1 scaffolding.** `CancelToken`, `dispatcher.cancel()`, and the in-flight table exist and are tested but **NEVER called from a production path**. v1 bounding is `cursor.execute(timeout=…)` plus explicit `cursor.close()` on cap exceedance (snowflake-connector's cursor has no public `cancel()`; `close()` acquires `_lock_canceling` and aborts the in-flight query). Do not add `notifications/cancelled` handlers, asyncio watchdogs, or signal handlers that invoke `dispatcher.cancel()`.
